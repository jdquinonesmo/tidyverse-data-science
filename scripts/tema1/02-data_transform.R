...
## * int -> n鷐eros enteros
## * dbl -> n鷐eros reales (double)
## * chr -> vector de caracteres o strings
## * dttm -> date + time 
## * lgl -> logical, contiene valores booleanos (T o F)
## * fctr -> factor, variables categ髍icas
## * date -> fecha (dia, mes y a駉)
...
## * group_by() -> opera la funci贸n a la que acompa帽a grupo a grupo
...
age.mery <- NA
#El tio John tambi茅n hace mucho que no lo veo, y no se cuantos a帽os debe tener...
age.john <- NA
#緿eben de tener la misma edad John y Mery?
age.mery == age.john
...
select(flights, one_of(v)) # Si no creara un vector con los nombres de las columnas o variables
                           ...
                           # con las comillas se separan los nombres y es 鷗il para no escribir siempre todas las variables en la funci髇 select(). 
...
# * Operaciones aritm茅ticas: +, -, *, /, ^  (hours + 60 * minutes)
# * Agregados de funciones: x/sum(x) : proporici贸n sobre el total
#                           x - mean(x): distancia respecto de media
#                           (x - mean(x))/sd(x): tipificaci贸n
#                           (x - min(x))/(max(x) - min(x)): estandarizar entre [0,1]
# * Aritm茅tica modular: %/%-> cociente de la divisi贸n entera, %% -> resto de la divisi贸n entera
#                       x == y * (x%/%y) + (x%%y) 
transmute(flights, 
          air_time,
